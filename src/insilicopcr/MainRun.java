...
public class MainRun extends Application {
	...
	public void start(Stage primaryStage) {
		...
		if(!Methods.checkVersion()) {
			pane.add(updateBox, 1, 48, 48, 2);
		}
			...
			public void handle(WindowEvent e) {
				...
				if(currentlyRunning) {
					if(JOptionPane.showConfirmDialog(null, 
						...
						if(!qaFile.exists()) {
							Methods.makeQALog(qaFile, version, outDir, inputFile, primerFile, BBToolsLocation, BLASTLocation);
						}
						try{
...
//			runBLASTTask(outputField);
			...
			Methods.makeQALog(new File(outDir.getAbsolutePath() + sep + "QAlog.txt"), version, outDir, inputFile, primerFile, BBToolsLocation, BLASTLocation);
	...
	public static void displayHelp() {
		...
		helpPane.getColumnConstraints().addAll(colCopies);
		RowConstraints rC = new RowConstraints();
		rC.setPercentHeight(5);
		List<RowConstraints> rowCopies = Collections.nCopies(20, rC);
		helpPane.getRowConstraints().addAll(rowCopies);
		
		Label helpLabel = new Label();
		
		helpPane.setGridLinesVisible(true);
