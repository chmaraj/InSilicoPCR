...
public class CommandMethods {
	...
	public static void makeExecutable(File BLASTLocation) {
		...
		try {
			Process p = Runtime.getRuntime().exec(processCall, null, BLASTLocation);
			try {
				p.waitFor();
	...
	public static void makeBlastDB(File reference, File BLASTLocation) {
		...
		String[] windowsFullProcessCall = {BLASTLocation.getAbsolutePath() + CommandMain.sep + "makeblastdb.exe", 
				"-dbtype", "nucl", "-parse_seqids", "-hash_index", "-in", in};
	...
	public static boolean checkVersion() {
		try{
			...
			try{
				...
				String currentVersion = String.join("", CommandMain.version.substring(1, CommandMain.version.length()).split("\\."));
